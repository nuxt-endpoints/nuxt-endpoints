...
| Stream response declarations                                          | Implemented                      | `{ stream: true, contentType, schema? }` keeps a streaming route inside `responses`: its status and media type reach OpenAPI, the payload is never validated, and the generated client stops parsing that route's body so callers receive the live stream                                                                                                                                                                                         |
