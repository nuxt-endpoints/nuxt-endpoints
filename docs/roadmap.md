...
| Contract-file separation for build-time discovery                     | Implemented                      | Nitro's contract macro keeps the contract expression and only the imports and immutable bindings it reaches, so handler-only code is never run during the build; non-endpoint routes produce no contract at all. Standard placement is a sibling `*.endpoint-contract.*` file, excluded from Nitro scanning through the public `ignore` option; the suffix is deliberately fixed (not configurable) because it exists only for that ignore pattern — contracts placed outside route directories need no suffix at all                                                                               |
...
| Cursor pagination contract                                            | Implemented (Nuxt 5 line)        | `pagination: { kind: 'cursor', item }` owns the generated request/page contract and projects it to Pinia Colada `infiniteQueryOptions()`                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Endpoint hooks (`onValidationError`, `wrapHandler`)                   | Implemented                      | Declared application-wide in `server/endpoints/runtime.ts`, the one file holding every setting `nuxt.config.ts` cannot carry. `onValidationError` resolves application -> default; `wrapHandler` nests application -> idempotency, making idempotency the built-in consumer of a now-public extension point rather than a privileged one                                                                                                                                                                                                                                                            |
| Application response contract composition                             | Implemented                      | `server/routes.config.ts` adds global, exact-path, trailing-`/**` prefix, and method response schemas to `$endpoint`, `useEndpoint`, Pinia Colada options, raw response types, and OpenAPI. It declares what middleware or infrastructure may return but does not register or inspect middleware; duplicate statuses become body unions and OpenAPI `oneOf`                                                                                                                                                                                                                                         |
| Framework failure client projection                                   | Implemented                      | Route-reachable validation (`400`/`406`/`415`) and idempotency (`400`/`409`/`422`) defaults are included in the same status union already documented by OpenAPI; custom hook outcomes use the route or application response contract                                                                                                                                                                                                                                                                                                                                                                |
...
| Per-method dispatch                                                   | Implemented                      | Method entries on a method-suffix-free `defineRouteHandler()` call, with derived HEAD/OPTIONS and 405 + Allow. All nine methods are declarable and an explicit `head`/`options` entry wins over the derived behaviour. Each entry carries its own contract, so metadata, idempotency, and media-type bodies work per method                                                                                                                                                                                                                                                                         |
...
| Declared response headers                                             | Implemented                      | Per-header-name schemas on a `{ body, headers }` or `{ media, headers }` member, in OpenAPI and validated when response validation is active. Body/header schema traversal defaults to development, is configurable as `always` / `development` / `never`, and does not disable the cheap declared-status check. A missing or rejected declared header is then a `500` contract breach; names are compared case-insensitively and an unset header validates as `undefined`. H3's `validateRouteContractResponse` ignores the `headers` member, so this stays downstream until that changes          |
...
The handler receives the h3 event itself, so standard Nitro middleware can
attach values to `event.context` and endpoint handlers read the same
...
export default defineRouteHandler({
  ...
  handler: (event) => findUserForAccount(event.context.user.accountId, event.validated.params.id),
})
```
...
Pinia Colada integration uses `queryOptions(request)` and
`mutationOptions(request)` from `#endpoints/colada`; the public guide documents
