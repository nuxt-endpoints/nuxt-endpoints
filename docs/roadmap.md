...
| Declared response headers                                             | Implemented                      | Per-header-name schemas on a `{ body, headers }` or `{ media, headers }` member, in OpenAPI and validated at runtime against the headers actually being sent. A missing or rejected declared header is a `500` contract breach, matching the body rule; names are compared case-insensitively and an unset header validates as `undefined`. H3's `validateRouteContractResponse` ignores the `headers` member, so this stays downstream until that changes                                                                                                                                          |
