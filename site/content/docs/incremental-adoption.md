...
- If the route directly default-exports a `defineRouteHandler({...})` call, it joins the generated client, the `#endpoints` types, and the OpenAPI document. Its schemas may be inline or imported from a [contract file](/docs/endpoints#separate-contract-files).
- If it does not, the route is skipped entirely — no contract is compiled for it. No validation is added, no types are generated for it, and its runtime behavior does not change.
