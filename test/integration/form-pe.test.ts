...
if (process.env.NUXT_ENDPOINTS_E2E === '1') {
  ...
  describe('progressive enhancement', async () => {
    ...
    it('renders the page with issues and submitted values in the same request', async () => {
      ...
      // Zod's issue details differ slightly between the versions available on
      // the two support lines; the code, path and server message are stable.
      expect(html).toContain('[too_small] name')
      expect(html).toContain('expected string to have')
      ...
      it(
        ...
        async () => {
          ...
          await page.waitForFunction(
            () =>
              (window as Window & { useNuxtApp?: () => { isHydrating: boolean } }).useNuxtApp?.()
                .isHydrating === false,
          )
