...
if (process.env.NUXT_ENDPOINTS_E2E === '1') {
  ...
  describe('Pinia Colada Nuxt integration', async () => {
    ...
    it('executes mutation options through Pinia Colada and pins the idempotency key', async () => {
      ...
      // A real `useMutation` accepted `{ key, mutation }` and ran the request.
      ...
      // Re-executing the same request object replayed the stored response
      // instead of running the handler again, so the generated key was reused.
