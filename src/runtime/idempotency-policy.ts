...
export type EndpointIdempotencyPolicy = {
   ...
   * another's recorded response. Use `global` only for a public operation
   * whose response contains no caller-specific data.
   */
  scope: IdempotencyGlobalScope | ((context: PolicyContext) => MaybePromise<string>)
  /**
   * `public` means no authorization is required; `middleware` means it was
   * completed before this handler. A callback is run for normal requests and
   * replays, including optional requests that carry no key.
   */
  authorization:
    | IdempotencyAuthorizationDelegation
    | IdempotencyPublicAuthorization
...
export function validateEndpointIdempotencyPolicy(
  ...
  if (unknown) {
    throw new TypeError(`Unknown idempotency policy option \`${unknown}\``)
  }
